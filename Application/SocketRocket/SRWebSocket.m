...
- (void)didConnect
{
    ...
    int ret = SecRandomCopyBytes(kSecRandomDefault, keyBytes.length, keyBytes.mutableBytes);
    assert(0 == ret);
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
    ...
    if (!useMask) {
        ...
        int ret = SecRandomCopyBytes(kSecRandomDefault, sizeof(uint32_t), (uint8_t *)mask_key);
        assert(0 == ret);
...
- (void)main;
{
    @autoreleasepool {
        ...
        NSTimer *timer = [[NSTimer alloc] initWithFireDate:[NSDate distantFuture] interval:0.0 target:self selector:@selector(nilSymbol) userInfo:nil repeats:NO];
